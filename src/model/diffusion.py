...
class AtomEmbedding(nn.Module):
    ...
    def forward(self, x,atoms):
        ...
        atom_ebd = self.atom_ebd(x[:,0:1].long()+1).squeeze()
...
class TensorProductScoreModel(torch.nn.Module):
    def __init__(self, args, model_params,
        ...
        num_atoms_protein = 20
        num_atoms_protein = 4 
        self.embed_Atoms_test=nn.Embedding(num_atoms_protein+1, args.ns)    
        self.atom_embed_protein = AtomEmbedding(args,num_atoms_protein)
        self.atom_embed_rna = AtomEmbedding(args, num_atoms_protein )
    ...
    def forward(self, batch):
        ...
        lig_node_attr = self.atom_embed_protein(ligand_graph[0],None)
        ...
        rec_node_attr = self.atom_embed_rna(receptor_graph[0],None )
...
# """
# Above model is for low noised one 
...
class TensorProductScoreModelold(torch.nn.Module):
    def __init__(self, args, model_params,
        confidence_mode=False,
        confidence_dropout=0,
        confidence_no_batchnorm=True,
        ...
        super(TensorProductScoreModelold, self).__init__()
        self.noise_schedule = NoiseSchedule(args)
        self.t_embedding = get_timestep_embedding(args)
    ...
    def forward(self, batch):
        ...
        # build ligand graph
        ligand_graph = self.build_rigid_graph(batch, "ligand")
        lig_node_attr = self.atom_embed(ligand_graph[0],None)
        lig_src, lig_dst = lig_edge_index = ligand_graph[1]
        lig_edge_attr = self.rec_edge_embed(ligand_graph[2])
        lig_edge_sh = ligand_graph[3]
        ...
        # build receptor graph
        receptor_graph = self.build_rigid_graph(batch, "receptor")
        rec_node_attr = self.atom_embed(receptor_graph[0],None)
        rec_src, rec_dst = rec_edge_index = receptor_graph[1]
        rec_edge_attr = self.rec_edge_embed(receptor_graph[2])
        rec_edge_sh = receptor_graph[3]
