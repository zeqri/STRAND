...
from Bio.Data.IUPACData import protein_letters_3to1,ambiguous_rna_values 
import fm
...
class Loader:
    ...
    def __init__(self, args, data=True):
        ...
        self.parser = Bio.PDB.MMCIFParser(QUIET=True)
        self.rna_list=ambiguous_rna_values.values() 
        self.protein_list=self.protein_letters()
    


    def protein_letters(self): 
        list=[]
        for key in protein_letters_3to1.keys():
            list.append(key.upper())    
        return list 
    ...
    def convert_pdb(self, all_res, all_atom, all_pos, all_visualization_values,pdb_id):
        """
        ...
        if atoms_to_keep is not None:
            ...
            # except:
            #     print("item which caused an error",pdb_id)
    ...
    def process_data(self, data, args):
        ...
        for item in data.values():
            ...
            for k in ["receptor", "ligand"]:
                ...
                if k in item:
                    subset = self.convert_pdb(*item[k],item["pdb_id"])
    ...
    def compute_embeddings(self, data):
        ...
        if os.path.exists(self.esm_cache) and self.args.recache  :    
        ...
        aa_code.update(
            {k.upper():v for k,v in ambiguous_rna_values.items()})
        ...
        rec_batches = self._esm_batchify(rec_seqs,fm_batch_converter)
        lig_batches = self._esm_batchify(lig_seqs, tokenizer)
    
        with torch.no_grad():
            pad_idx = alphabet.padding_idx
            fm_pad_idx = fm_alphabet.padding_idx
            rec_reps = self._rna_run_esm(rec_batches, fm_pad_idx)      
    ...
    def _rna_run_esm(self, batches, padding_idx):
        """
            Wrapper around ESM specifics
            @param (list)  batch
            @return (list)  same order as batch
        ...
        #for batch in tqdm(batches, desc="ESM", ncols=50):
        for batch in batches:            
            # if rna length greater that 1022 chunk and concatenate
            if batch.shape[1] > 1022 : 
                chunks=split_tensor(batch,1022)   
                all_embeddings = []
                for chunk in chunks:                    
                    reps = self.rna_model(chunk.cuda(), repr_layers=[12])
    ...
    def unique_chain_id(self,df):
        unique_chain_id=df["chain"].unique()
        data={}

        for chain_id in unique_chain_id:  
            chain = df[df["chain"] == chain_id]
            data[chain_id] = chain

        return data
    
# Function to parse PDB file and convert to Pandas DataFrame

    def pdb_to_dataframe(self,pdb_file):
        ...
        for model in structure:
            for chain in model:
                for residue in chain:
                    # if residue.get_resname().strip() in self.rna_list:
                    ...
                    elif residue.get_resname().strip() in self.protein_list:
    ...
    def parse_df(self,df):
        ...
        all_pos=torch.tensor([df["x"].values,df["y"].values,df["z"].values] ).t(  )
    ...
    def parse_dill( self,name,df):
        ...
        for type in types:
            p = df[df["type"] == type].reset_index(drop=True)
            if type == "rna":
                p= self.parse_df(p) 
                item["receptor"] = p        
            elif type == "protein":     
                ...
                print(f"data point  has no rna and protien")
...
class RNALoader(Loader):
    ...
    def read_files(self):
        ...
        if  self.args.recache and os.path.exists(self.data_cache)  : # load from cache if path exists
            printt("Loading cached data")
        ...
        for line in tqdm(self.data_to_load,
            ...
            if item is None:
                with open(os.path.join(self.root, path), 'rb') as file: 
                    df= dill.load(file )   
                item = self.parse_dill(path, df)   # parse dill file
                path_to_data[path] = item #
            item.update(line) 
            data[path] = item # add to data
            if self.args.debug:
                if len(data) >= 2:
                    break
        # write to cache
...
def split_into_folds(data, args):
    ...
    print(f"Splitting into {args.num_folds} folds")
...
def split_tensor(tensor, max_length):
    _ , seq_length = tensor.shape
    chunks = []

    # Compute the number of chunks
    num_chunks = (seq_length + max_length - 1) // max_length
